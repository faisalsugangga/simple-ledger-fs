...
export function TransactionForm({
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      if (transactionToEdit) {
        ...
        setEntries(
          transactionToEdit.entries.map((entry) => ({
            accountId: String(entry.account_id),
            debit: entry.type === "debit" ? String(entry.amount) : "",
            credit: entry.type === "credit" ? String(entry.amount) : "",
          }))
        );
      } else {
        setDescription("");
  ...
  }, [isOpen, supabase, transactionToEdit]);
  ...
  const handleSubmit = async () => {
    if (!isFormValid) {
        toast.error("Silakan lengkapi semua bidang yang diperlukan.");
        return;
    }
  ...
  const handleFinalSubmit = async () => {
    ...
    let result;
    if (transactionToEdit) {
      result = await updateJournalTransaction(
      ...
      result = await addJournalTransaction(description, date, formattedEntries);
    }
  ...
  const formTitle = transactionToEdit
    ? "Ubah Jurnal Transaksi"
    : "Tambah Jurnal Transaksi Baru";
  ...
  return (
    <>
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="mt-4 border-t pt-4">
              ...
              <Button variant="outline" size="sm" onClick={addEntry} className="mt-2">
                <PlusCircle className="h-4 w-4 mr-2" /> Tambah Baris
              </Button>
            </div>
      ...
      <AlertDialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Transaksi Tidak Seimbang</AlertDialogTitle>
            <AlertDialogDescription>
              Transaksi ini tidak seimbang. Selisih Debit dan Kredit adalah sebesar{" "}
              ...
              <br />
              <br />
              Apakah Anda yakin ingin melanjutkan dan menyimpan transaksi ini?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Batalkan</AlertDialogCancel>
            <AlertDialogAction onClick={handleFinalSubmit}>Lanjutkan</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </>
